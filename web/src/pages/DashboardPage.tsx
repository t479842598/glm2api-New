...
function RecentActivityCard({
  ...
  const total = data?.recent_total ?? 0
  const success = data?.recent_success ?? 0
  const error = data?.recent_error ?? 0
  ...
  const avgMs = data?.recent_avg_ms ?? 0
...
function RecentErrorRow({
  ...
  const message = item.error_message || "请求异常"
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <p className="mt-0.5 truncate text-[11px] text-muted-foreground">
          {item.api_key_name || item.glm_account || "-"} · {message}
        </p>
      </div>
...
function QuickActionsCard() {
  ...
  const actions = [
    ...
    },
    {
      ...
      onClick: () => navigate("/admin/app-logs"),
    },
  ]
...
export default function DashboardPage() {
  const { data, loading, error } = usePolling<DashboardStats>(api.stats, 30000)
    ...
    <div className="mx-auto w-full max-w-[1320px] space-y-5">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <StatCard
          ...
          value={
            data ? (
              <div className="flex min-w-0 items-center gap-2">
                <span className="truncate">{data.token_total} 个</span>
                <Badge
                  variant={data.token_guest_mode ? "secondary" : "default"}
                  className="text-[10px]"
                >
                  {data.token_guest_mode ? "游客" : "Token"}
                </Badge>
              </div>
            ) : (
              "-"
          ...
          detail={data?.token_source ? `来源：${data.token_source}` : undefined}
          loading={loading}
        />
        <StatCard
          icon={<Key className="size-4" />}
          title="API Keys"
          value={data ? `${data.key_active}/${data.key_count}` : "-"}
          detail={
            data
              ? data.auth_enabled
                ? "接口已启用 Key 认证"
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1fr)_360px]">
        ...
        <div className="space-y-5">
          <RuntimePolicyCard data={data ?? undefined} loading={loading} />
          <QuickActionsCard />
          <Card className="border-border/60 shadow-sm">
            <CardContent className="pt-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium">模型数量</p>
                  <p className="text-xs text-muted-foreground">
                    当前对外暴露 {data?.model_count ?? "-"} 个模型
                  </p>
                </div>
              </div>

...
def _record_request(
    ...
    detail_store: RequestDetailStore,
    *,
    method: str,
    path: str,
    model: str = "",
    ...
    detail_store.add(RequestDetailRecord(
        method=method, path=path, model=model,
        status_code=status, duration_ms=duration_ms, error=error,
...
def _serve_admin_web_static(handler, path: str) -> None:
    """Serve static files from the admin_web directory (Vite build output)."""
    from pathlib import Path

    # /admin/assets/xxx.js → admin_web/assets/xxx.js
...
class GLM2APIServer:
    ...
    def _build_handler(self):
        ...
        request_store = RequestLogStore()
        request_detail_store = RequestDetailStore()
        ...
        class RequestHandler(BaseHTTPRequestHandler):
            ...
            _admin_request_detail_store = request_detail_store
            _admin_api_key_store = api_key_store
            _admin_start_time = start_time
            ...
            def do_GET(self) -> None:
                ...
                try:
                    ...
                    if path == f"{config.api_prefix}/models":
                        ...
                        _record_request(request_store, request_detail_store, method="GET", path="/v1/models", status=200, duration_ms=(time.time() - _start) * 1000)
                        return
                    ...
                    # ── Admin web static files (assets) ──────────────────
                    if path.startswith("/admin/assets/"):
                        _serve_admin_web_static(self, path)
                        return
                    ...
                    if path.startswith("/admin/api/app-logs"):
                        handle_admin_app_logs(self)
                        return

                    if path.startswith("/admin/api/request-logs"):
                        # 处理详情页路由 /admin/api/request-logs/:id
                        rest = path[len("/admin/api/request-logs"):]
                        if rest.startswith("/") and len(rest) > 1:
                            handle_admin_request_log_detail(self, rest[1:])
                        else:
                            handle_admin_request_logs(self)
                        return
                    ...
                    _record_request(request_store, request_detail_store, method="GET", path=path, status=404, duration_ms=(time.time() - _start) * 1000, error="Not Found")
            ...
            def do_POST(self) -> None:
                ...
                try:
                    ...
                    if path == "/admin/api/request-logs/clear":
                        handle_admin_request_logs_clear(self)
                        return
